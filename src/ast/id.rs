...
pub struct Id<T: ArenaItem>(usize, PhantomData<T>);

impl<T: ArenaItem> Id<T> {
    pub fn new(id: usize) -> Self {
        Id(id, PhantomData)
    }

    /// Decode id to get the [ArenaItem] value.
    pub fn de(self, arena: &impl GetArena) -> T {
        T::get_id_map(arena.get_arena()).get(self).unwrap()
    }
}

impl<T: ArenaItem> fmt::Debug for Id<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Id::<{}>({})", std::any::type_name::<T>(), self.0)
    }
}

impl<T: ArenaItem> Clone for Id<T> {
    fn clone(&self) -> Self {
        Self::new(self.0)
    }
}

impl<T: ArenaItem> Copy for Id<T> {}

impl<T: ArenaItem> PartialEq for Id<T> {
    fn eq(&self, other: &Self) -> bool {
        self.0 == other.0 && self.1 == other.1
    }
}

impl<T: ArenaItem> Eq for Id<T> {}

impl<T: ArenaItem> LiteralValue for Id<T> {
    fn literal_value(&self) -> String {
        format!("Id::new({})", self.0)
    }
}

impl<T: ArenaItem> Hash for Id<T> {
...
impl<T: ArenaItem> Default for IdMap<T> {
    fn default() -> Self {
        IdMap::new()
    }
}

impl<T: ArenaItem> IdMap<T> {
    pub fn new() -> Self {
        IdMap { map: Vec::new() }
    }
